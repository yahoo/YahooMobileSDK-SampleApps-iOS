///
/// @file BannerViewController.swift
...
class BannerViewController : UIViewController {
    ...
    @IBAction func requestInline(_ sender: AnyObject) {
        ...
        ad = YASInlineAdView(placementId: AdPlacementData.bannerId)
        ad?.delegate = self
        ad?.load(with: nil)
    }
...
extension BannerViewController: YASInlineAdViewDelegate {
    ...
    func inlineAdDidFail(_ inlineAd: YASInlineAdView, withError errorInfo: YASErrorInfo) {
        print("Inline ad failed with error: \(errorInfo)")
    }
